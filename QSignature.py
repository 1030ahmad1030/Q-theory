...
def tau_s(t: np.ndarray, R: np.ndarray, R_inf: Optional[float] = None, 
    ...
    """
    τ_s: Signed centroid timescale (FIXED VERSION).
    
    FIXED: Uses step_height = R_inf - R[0] instead of just R_inf
    Matches paper's assumption R(0)=0, R(∞)=1
    """
    t = np.asarray(t, dtype=float)
    R = np.asarray(R, dtype=float)
    ...
    # Use step height, not R_inf (FIX)
    step_height = R_inf - R0
    
    if abs(step_height) < eps:
        # If step height is tiny, treat as centered signal
        R_centered = R - np.mean(R)
        return tau_s(t, R_centered, R_inf=0.0, eps=eps)
    ...
    return integral / step_height  # FIXED: divide by step height


def tau_2(t: np.ndarray, R: np.ndarray, R_inf: Optional[float] = None) -> float:
    """
    τ²: Step-response timescale (FIXED VERSION).
    
    FIXED: Normalizes to step height R_inf - R[0]
    Matches paper's assumption R(0)=0, R(∞)=1
    """
    t = np.asarray(t, dtype=float)
    R = np.asarray(R, dtype=float)
    ...
    # Normalize response: (R - R0)/step_height goes from 0 to 1
    R_norm = (R - R0) / step_height
    
    # Now R_norm goes 0→1 as in paper Eq. 5
    f = 1 - R_norm
    return np.trapezoid(f, t)
    
    # FIX: Also check denominator magnitude
    if abs(R_inf) < eps:
        return 0.0
...
def tau_3(t: np.ndarray, R: np.ndarray, 
    ...
    Methods:
    - 'autocorrelation': For centered signals (mean ≈ 0) - original method
    - 'autocorr_centered': For step responses (center by R_inf) - FIXED
    - 'impulse': For impulse responses (Theorem 3.4)
    """
    t = np.asarray(t, dtype=float)
    ...
    if method in ['autocorrelation', 'autocorr_centered']:
        ...
        if method == 'autocorr_centered':
            # FIX: For step responses, center by R_inf
            if R_inf is None:
                R_inf = R[-1]
            R_centered = R - R_inf
        else:
            # Original method: center by mean
            R_centered = R - np.mean(R)
        ...
        raise ValueError(f"Method must be 'autocorrelation', 'autocorr_centered', or 'impulse', got {method}")
...
def rho_13_step(t: np.ndarray, R: np.ndarray, R_inf: Optional[float] = None) -> float:
    ...
    """
    tau_s_val = tau_s(t, R, R_inf)
    tau_3_val = tau_3(t, R, method='autocorr_centered', R_inf=R_inf)  # ← FIX!
...
def compute_all(t: np.ndarray, R: np.ndarray, R_inf: Optional[float] = None,
    ...
    results['tau_3'] = tau_3(t, R, method='autocorr_centered', R_inf=R_inf)  # FIXED LINE
